...
class CPPAddon : public StreamingWorker {
  public:
    CPPAddon(Callback *data
      , Callback *complete
      ...
      do {
        ...
        if ( filter_by_name(m.name) || value <= 0) {
          if ( value > 0 ){
            ...
            Message tosend("TEST", std::to_string(sum));
...
StreamingWorker * create_worker(Callback *data
 ...
 return new CPPAddon(data, complete, error_callback, options);
}

NODE_MODULE(cppaddon, StreamWorkerWrapper::Init)